...
IK_BEZIER_SUPPORT_START = 0.08   # delay support push slightly inside the Bezier swing
...
def _build_bezier_tripod_frames(
    ...
    for i in range(1, steps + 1):
        ...
        if t <= IK_BEZIER_SUPPORT_START:
            support_t = 0.0
        else:
            support_t = _smoothstep((t - IK_BEZIER_SUPPORT_START) / max(0.001, 1.0 - IK_BEZIER_SUPPORT_START))
...
def action_ready(bus: DynamixelBus, use_safety_check: bool = True):
    ...
    if not (BODY_IK_ENABLED and body_ik_has_nonzero_pose()):
        return _PREVIOUS_ACTION_READY_V15(bus, use_safety_check)
...
#     2) efficient IK gait         -> current best reach/speed native IK, Bezier OFF
#     3) Bezier IK gait            -> smoother motor-friendly curved foot arc
...
#   - `ik efficient` is the best practical walking mode for distance/speed.
#   - `ik bezier_default` is smoother and gentler, but less distance-efficient.
# ============================================================
...
def apply_motion_profile(profile: str):
    ...
    if p in ["fixed", "hardcoded", "raw", "legacy", "default"]:
        ...
        BODY_IK_ENABLED = False
        CURRENT_MOTION_PROFILE = "fixed"
        return "Fixed / hardcoded gait selected. Original joint-space movement will be used."

    if p in ["ik", "efficient", "native", "normal", "fast"]:
        IK_ENABLED = True
        IK_NATIVE_GAIT = True
        IK_NATIVE_BEZIER_ENABLED = False
        BODY_IK_ENABLED = False
        CURRENT_MOTION_PROFILE = "ik-efficient"
        return "Efficient native IK selected. Uses IK foot targets without Bezier curve. Best reach/speed mode."

    if p in ["bezier", "curve", "smooth", "arc"]:
        IK_ENABLED = True
        IK_NATIVE_GAIT = True
        IK_NATIVE_BEZIER_ENABLED = True
        BODY_IK_ENABLED = False
        CURRENT_MOTION_PROFILE = "ik-bezier"
        return "Bezier IK selected. Uses curved foot arc. Smoother/gentler but less distance-efficient."

    if p in ["bezier_showcase", "showcase", "big_curve", "obvious_curve", "curve_show"]:
        IK_ENABLED = True
        IK_NATIVE_GAIT = True
        IK_NATIVE_BEZIER_ENABLED = True
        BODY_IK_ENABLED = False
        ...
        IK_STEP_CM = 6.5
        IK_SUPPORT_PUSH_CM = 3.4
        IK_LIFT_CM = 7.5
        IK_TURN_DEG = 18.0
        IK_LIFT_TARGET_SCALE = 1.28
        IK_NATIVE_LIFT_EXTRA_HOLD = 0.32
        IK_BEZIER_STEPS = 14
        IK_BEZIER_FRAME_DELAY = 0.035
        IK_BEZIER_ARC_EXTRA_CM = 3.2
        IK_NATIVE_EXTRA_END_HOLD = 0.050
        CURRENT_MOTION_PROFILE = "ik-bezier-showcase"
        return "Bezier SHOWCASE selected. Bigger visible foot arc; smoother but slower/less distance-efficient."
    ...
    if p in ["bodyik", "body", "posture"]:
        ...
        IK_NATIVE_BEZIER_ENABLED = False
        BODY_IK_ENABLED = True
        CURRENT_MOTION_PROFILE = "bodyik-test"
        return "Body IK posture test selected. Use bodyik height/pitch/roll then r."

    return "Unknown profile. Use: fixed, ik, bezier, bodyik."


def print_motion_profile_status():
    print()
    print("===================================================")
    print(" MOTION PROFILE SWITCHES / V18")
    print("===================================================")
    ...
    print("  ik efficient         # best IK reach/speed, Bezier OFF")
    print("  ik bezier_default    # smoother Bezier IK with saved good settings")
    print("  ik bezier_showcase   # exaggerated curve to clearly see Bézier foot arc")
...
def action_ik_settings(parts: List[str]):
    ...
    if sub in ["fixed", "hardcoded", "raw", "legacy", "default"]:
        ...
        return
    if sub in ["efficient", "native_default", "normal", "fast_default"]:
        print(apply_motion_profile("ik"))
        return
    if sub in ["bezier_default", "curve_default", "smooth_default"]:
        print(apply_motion_profile("bezier"))
        return
    if sub in ["bezier_showcase", "showcase", "big_curve", "obvious_curve", "curve_show"]:
    ...
    if sub in ["bezier", "curve", "arc"] and len(parts) >= 3:
        ...
        if value in ["on", "true", "1", "yes"]:
            ...
            IK_NATIVE_BEZIER_ENABLED = True
            CURRENT_MOTION_PROFILE = "ik-bezier"
            print("IK Bezier foot arc ON. Current profile = ik-bezier.")
            return
        if value in ["off", "false", "0", "no"]:
            IK_NATIVE_BEZIER_ENABLED = False
            CURRENT_MOTION_PROFILE = "ik-efficient" if IK_ENABLED else "fixed"
            print("IK Bezier foot arc OFF. Current profile = ik-efficient if IK is enabled.")
            return
...
_WEB_PROFILE_SECTION = """<div class=\"section\"><h2>Movement Mode Switches</h2><div class=\"sub\">Switch between hardcoded fixed gait, efficient IK, and smoother Bézier IK. These do not move the robot by themselves.</div><div class=\"presetgrid\"><button id=\"btn_mode_fixed\" class=\"btn small\" onclick=\"presetCmd(this,'ik fixed')\">Fixed / Hardcoded</button><button id=\"btn_mode_ik\" class=\"btn small\" onclick=\"presetCmd(this,'ik efficient')\">IK Efficient</button><button id=\"btn_mode_bezier\" class=\"btn small\" onclick=\"presetCmd(this,'ik bezier_default')\">IK Bézier Smooth</button><button class=\"btn small\" onclick=\"presetCmd(this,'ik bezier_showcase')\">Bézier Showcase</button><button class=\"btn small\" onclick=\"presetCmd(this,'ik bezier on')\">Bezier ON</button><button class=\"btn small\" onclick=\"presetCmd(this,'ik bezier off')\">Bezier OFF</button><button id=\"btn_mode_bodyik\" class=\"btn small\" onclick=\"presetCmd(this,'ik bodyik')\">Body IK Test</button><button class=\"btn small\" onclick=\"presetCmd(this,'ik')\">IK Status</button><button class=\"btn small\" onclick=\"presetCmd(this,'bodyik')\">BodyIK Status</button><button class=\"btn small\" onclick=\"presetCmd(this,'health')\">Health Refresh</button></div></div>"""
...
try:
    ...
    WEB_HTML = WEB_HTML.replace("<span class=\"pill\">End: ${s.timing.end_mode}</span>", "<span class=\"pill\">End: ${s.timing.end_mode}</span><span class=\"pill\">IK Profile: ${s.ik?s.ik.profile:'--'}</span><span class=\"pill\">Bezier: ${s.ik?s.ik.bezier:false}</span><span class=\"pill\">IK Lift: ${s.ik?s.ik.lift:'--'} cm</span>")
