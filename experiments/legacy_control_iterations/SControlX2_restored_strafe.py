...
def side_strafe_final_recenter(bus: DynamixelBus, direction: str):
    ...
    CURRENT_MODE = "READY_REFINED2K"
    send_phase_side_legacy(bus, dict(READY_POSE), GAIT_SPEED, GAIT_FINAL_READY_DELAY)
    ACTIVE_GOALS = dict(READY_POSE)
        ...
        while True:
            ...
            try:
                if cmd == "x":
                    ...
                    # Continuous A/D side strafe, like W/S. Press Enter to stop.
                    direction = normalize_direction(cmd)
                    action_side_strafe_continuous(bus, direction)
