...
# VERSION: WCONTROL23 - NO-HIP LIFT-OUT + PLANTED-PULL A/D STRAFE
#
# Main updates:
...
def action_ready(bus: DynamixelBus, use_safety_check: bool = True):
    ...
    print()
    print("ACTION: RETURN TO REFINED2K BALANCED READY_POSE")
...
def build_side_strafe_targets(
    ...
    if phase == "up_pull":
        ...
        for leg in other_tripod:
            h, f, t = side_strafe_leg_offsets(leg, direction, "pull_ground")
            targets.update(build_leg_offset_targets(leg, h, f, t))
...
def print_side_strafe_settings():
    print()
    print("===================================================")
    print(" WCONTROL23 A/D NO-HIP LIFT-OUT + PLANTED-PULL SETTINGS")
    print("===================================================")
...
def action_side_strafe_settings(parts: List[str]):
    ...
    try:
        if sub == "flip":
            ...
            SIDE_STRAFE_FEMUR_REACH_DEG = max(-18.0, min(18.0, float(parts[2])))
            SIDE_STRAFE_TIBIA_REACH_DEG = max(-18.0, min(18.0, float(parts[3])))
            ...
            print("A/D strafe hip movement is locked to 0. This WControl21 version intentionally uses NO hips.")
            return
        elif sub == "lift" and len(parts) == 4:
            # User enters positive numbers: sidestrafe lift 34 -6 means femur -38, tibia +28.
            SIDE_STRAFE_LIFT_FEMUR_DEG = -abs(max(20.0, min(48.0, float(parts[2]))))
            SIDE_STRAFE_LIFT_TIBIA_DEG = abs(max(12.0, min(40.0, float(parts[3]))))
        elif sub == "hold" and len(parts) == 3:
            SIDE_STRAFE_HOLD = max(0.05, min(0.80, float(parts[2])))
        else:
            print("Usage: sidestrafe / sidestrafe flip / sidestrafe reach 6 -14 / sidestrafe pull -5 12 / sidestrafe lift 34 -6 / sidestrafe hold 0.25")
            return
    except ValueError:
...
def print_help():
    ...
    print(" WCONTROL20 TRIPOD SIDE-STEP A/D STRAFE + PRECISION TURN + V6 SPLIT-LIFT")
    print("===================================================")
...
def main():
    ...
    try:
        print()
        print("Moving to REFINED2K balanced READY_POSE first...")
