...
# WEB LAYER RESTORED FROM KNOWN-WORKING CONTROL SERVER
# ============================================================
...
def web_run_terminal_command(bus: DynamixelBus, raw_cmd: str):
    ...
    # Web terminal convenience: allow several commands separated by semicolon or newline.
    # Example: ikinfo; ikbig FL lift 3; ikreset FL
    if ";" in raw_cmd or "\n" in raw_cmd:
        results = []
        for sub_cmd in raw_cmd.replace("\r", "\n").replace(";", "\n").split("\n"):
            sub_cmd = sub_cmd.strip()
            if not sub_cmd:
                continue
            result = web_run_terminal_command(bus, sub_cmd)
            results.append(result)
            if isinstance(result, dict) and not result.get("ok", True):
                return result
        return {"ok": True, "message": f"Ran {len(results)} command(s)"}
    ...
    try:
        if cmd in ["x", "exit", "quit"]:
            ...
            # Web terminal support for the research/formula model display.
            # Terminal mode already supported this; web mode was missing it,
            # causing: Unknown command: formula forward.
            capture_to_web_log(print_research_model, parts[1] if len(parts) >= 2 else "forward")

        # --------------------------------------------------
        # WEB TERMINAL IK COMMAND ROUTER
        # --------------------------------------------------
        # The terminal mode already supported IK commands, but the web terminal
        # only routed original SControlX2 commands. This block forwards IK
        # commands to the same action_* functions and captures their print output
        # into the browser log.
        elif cmd in ["th", "terminalhelp", "terminal_help", "ikhelp"]:
            capture_to_web_log(print_terminal_mode_help)
        elif cmd == "ikinfo":
            capture_to_web_log(print_ik_info)
        elif cmd == "ikcalc":
            capture_to_web_log(action_ik_calc, parts)
        elif cmd == "ikmove":
            capture_to_web_log(action_ik_move, bus, parts)
        elif cmd == "iklift":
            capture_to_web_log(action_ik_lift, bus, parts, True)
        elif cmd == "ikforward":
            capture_to_web_log(action_ik_forward, bus, parts, True)
        elif cmd == "ikbig":
            capture_to_web_log(action_ik_big, bus, parts, True)
        elif cmd == "ikbigcalc":
            capture_to_web_log(action_ik_big, bus, parts, False)
        elif cmd in ["ikreset", "legready"]:
            capture_to_web_log(action_ik_reset_leg, bus, parts)
        elif cmd in ["ikphase", "ikstand"]:
            capture_to_web_log(action_ik_phase, bus, parts)
            ...
            if cmd == "ikinspect4" and len(parts) == 1:
                parts = ["ikinspect", "4"]
            capture_to_web_log(action_ik_inspect_gait, bus, parts)
        elif cmd in ["ikresetall", "ikallready"]:
            capture_to_web_log(action_ik_reset_all_command, bus, parts)
        elif cmd == "iktripod":
            capture_to_web_log(action_ik_tripod, bus, parts)
        elif cmd == "ikgait":
            capture_to_web_log(action_ik_gait, bus, parts, 1)
        elif cmd == "ikwalk":
            capture_to_web_log(action_ik_walk, bus, parts)
        elif cmd in ["ikstep4", "ikgait4", "ikwalk4", "ikfullwalk4"]:
            if cmd in ["ikstep4", "ikgait4"] and len(parts) == 1:
                parts = [cmd, "1"]
            capture_to_web_log(action_ik_walk4, bus, parts)
        elif cmd in ["ikwalk4pause", "ikpausewalk4", "ikwalkpause4"]:
            capture_to_web_log(action_ik_walk4_pause, bus, parts)
        elif cmd in ["ikgait3", "ikfulltest3", "ikfullgait3", "iktest3"]:
            capture_to_web_log(action_ik_full_gait_level3, bus, parts)
...
def create_web_app(bus: DynamixelBus):
    try:
        ...
        # IMPORTANT for this IK file:
        # The file uses `from __future__ import annotations`, so endpoint
        # annotations like `request: Request` are stored as strings.
        # FastAPI resolves those strings using the module globals, not this
        # function's locals. Put Request into globals before routes are
        # decorated; otherwise FastAPI may treat request as a body field and
        # return 422 before the endpoint runs.
    ...
    print("[WEB API PATCH] Request annotation resolved globally; /api/command should not 422.")
    print("[WEB IK PATCH] Web terminal now routes ikinfo/ikbig/ikgait/ikstep4/ikwalk4 commands.")
    ...
    async def _payload(request: Request) -> Dict[str, Any]:
        """Accept JSON, form, query string, or raw text so web buttons/controllers do not 422."""
        data: Dict[str, Any] = {}
        try:
    ...
    async def api_command(request: Request):
        ...
        command = _pick(data, "command", "cmd", "text", "value", "c", default="")
        command = str(command).strip()
        if not command:
            web_log(f"WEB command ignored: missing command payload {data}")
            return {"ok": False, "message": "missing command", "received": data}
        with WEB_BUSY_LOCK:
            return web_run_terminal_command(bus, command)

    @app.post("/api/move/start")
    async def api_move_start(request: Request):
        data = await _payload(request)
        direction = _pick(data, "direction", "dir", "move", "command", "cmd", default="")
        direction = normalize_direction(str(direction).strip())
        if not direction:
            web_log(f"WEB move ignored: missing direction payload {data}")
            return {"ok": False, "message": "missing direction", "received": data}
        return web_start_motion(bus, direction)

    @app.post("/api/move/stop")
    async def api_move_stop(request: Request):
        web_stop_motion()
        return {"ok": True, "message": "Stop requested"}

    @app.post("/api/action/bodylevel")
    async def api_action_bodylevel(request: Request):
        data = await _payload(request)
        if WEB_MOTION_THREAD and WEB_MOTION_THREAD.is_alive():
        ...
        mode = str(_pick(data, "mode", default="set") or "set").lower()
        try:
            level = int(_pick(data, "level", "value", default=0))
        except Exception:
            level = 0
        try:
            delta = int(_pick(data, "delta", default=0))
        except Exception:
            delta = 0
        with WEB_BUSY_LOCK:
            if mode == "reset":
                ...
                ok = action_body_level_delta(bus, delta)
            else:
                ok = action_body_level_set(bus, level, True)
        return {"ok": bool(ok), "message": f"bodylevel {BODY_HEIGHT_LEVEL}", "level": BODY_HEIGHT_LEVEL}

    @app.post("/api/action/pushup")
    async def api_action_pushup(request: Request):
        data = await _payload(request)
        if WEB_MOTION_THREAD and WEB_MOTION_THREAD.is_alive():
        ...
        try:
            level = int(_pick(data, "level", "value", default=1))
        except Exception:
            level = 1
        with WEB_BUSY_LOCK:
            ok = action_pushup_quick(bus, str(level))
        return {"ok": bool(ok), "message": f"pushup {level}"}

    @app.post("/api/action/liftall")
    async def api_action_liftall(request: Request):
        data = await _payload(request)
        if WEB_MOTION_THREAD and WEB_MOTION_THREAD.is_alive():
        ...
        try:
            level = int(_pick(data, "level", "value", default=7))
        except Exception:
            level = 7
        with WEB_BUSY_LOCK:
            ok = action_lift_all_quick(bus, level)
        return {"ok": bool(ok), "message": f"liftall {level}"}
    return app
