...
# ============================================================
# ROBUST AUTOMATED FEMUR LIMIT / MOVEMENT PROBE
# ============================================================
#
# Purpose:
#   Fully automatic test for all 6 femur joints.
#
# It will:
#   1. Connect to COM6
#   2. Capture current pose as SESSION_READY
#   3. Test all femurs automatically
#   4. Move each femur positive direction
#   5. Return to center and wait properly
#   6. Move each femur negative direction
#   7. Return to center and wait properly
#   8. Print every movement target + feedback
#   9. Print final paste summary
#
# IMPORTANT:
#   MR and RR femur movement is reversed because your robot's
#   leg 5 and leg 6 assembly is physically opposite.
#
# Run:
#   python femur_auto_probe.py --yes
#
# Safer:
#   python femur_auto_probe.py --step 2 --max-angle 20 --speed 35 --settle 0.5 --yes
#
# Stronger:
#   python femur_auto_probe.py --step 3 --max-angle 30 --speed 40 --settle 0.5 --yes
#
# Test one femur only:
#   python femur_auto_probe.py --joints MR_femur --yes
#
# ============================================================
...
#
# You discovered:
#   MR and RR femur/tibia move physically opposite.
# ============================================================
...
LEG_MOVEMENT_SIGN = {
    ...
    # Legs 5 and 6 reversed for femur/tibia.
...
def return_all_femurs_to_ready(bus: DynamixelBus, speed: int):
    targets = {}

    for joint_name in FEMUR_JOINTS:
        motor_id = int(JOINT_LIMITS[joint_name]["id"])
...
def probe_joint(
    ...
    print("\n\n###################################################")
    print(f"FEMUR JOINT PROBE: {joint_name}")
    print("###################################################")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Robust automated femur probe with detailed feedback.")

    parser.add_argument("--port", default=DEFAULT_PORT, help="COM port. Default COM6.")

    parser.add_argument("--step", type=float, default=DEFAULT_STEP_DEG, help="Step degrees. Default 3.")
    parser.add_argument("--max-angle", type=float, default=DEFAULT_MAX_ANGLE, help="Max probe angle each side. Default 30.")
    parser.add_argument("--speed", type=int, default=DEFAULT_SPEED, help="Moving speed. Default 40.")
    parser.add_argument("--settle", type=float, default=DEFAULT_SETTLE_SECONDS, help="Seconds after each step. Default 0.50.")

    parser.add_argument(
        "--joints",
        nargs="*",
        default=None,
        help="Optional femur list. Example: --joints FL_femur MR_femur",
    )
...
def main():
    ...
    selected_joints = FEMUR_JOINTS

    if args.joints:
        selected_joints = []

        for joint in args.joints:
            if joint not in FEMUR_JOINTS:
                print(f"Invalid femur joint: {joint}")
                print(f"Allowed: {FEMUR_JOINTS}")
                return

            selected_joints.append(joint)

    print("\n===================================================")
    print(" HEXAPOD ROBUST AUTOMATED FEMUR PROBE")
    print("===================================================")
    print(f"Port      : {args.port}")
    print(f"Joints    : {selected_joints}")
    ...
    print("MR and RR femur signs are reversed automatically.")
    print("===================================================")
    ...
    if not args.yes:
        confirm = input("Type y to start femur automatic probe: ").strip().lower()
    ...
    try:
        ...
        print("\nReturning all femurs to captured ready before starting...")
        return_all_femurs_to_ready(bus, speed=args.speed)
        ...
        for joint in selected_joints:
            ...
            print("\nReturning all femurs to ready before next joint...")
            try:
                return_all_femurs_to_ready(bus, speed=args.speed)
            except Exception as e:
                print(f"Return all femurs warning: {type(e).__name__}: {e}")

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        step_csv = f"femur_probe_steps_{timestamp}.csv"
        summary_csv = f"femur_probe_summary_{timestamp}.csv"
