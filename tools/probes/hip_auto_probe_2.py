...
# ============================================================
# ROBUST AUTOMATED HIP LIMIT / MOVEMENT PROBE
# ============================================================
#
# IMPORTANT FIX:
# This script ignores old joint-specific raw limits from
# hexapod_kinematics.py.
#
# It ONLY clamps to Dynamixel absolute range:
#   raw 0 to raw 1023
#
# This fixes MR_hip / ID 7 where old max_raw=598 caused:
#   target_raw=598
#   target_raw=598
#   target_raw=598
#
# Correct behavior after this fix should be:
#   target_raw=662
#   target_raw=690
#   target_raw=717
#
# Run MR_hip only:
#   python hip_auto_probe.py --hips MR_hip --step 8 --max-angle 95 --speed 65 --settle 0.35 --debug-clamp --yes
#
# Run all hips:
#   python hip_auto_probe.py --step 8 --max-angle 95 --speed 65 --settle 0.35 --yes
#
# ============================================================
...
# MOVEMENT SIGN
# ============================================================
# Legs 5 and 6 are reversed for femur/tibia only.
# Hips stay normal.
# ============================================================
...
def clamp_raw_for_joint(joint_name: str, raw: int) -> int:
    """
    For probing, ignore old joint-specific raw limits.
    Only clamp to Dynamixel absolute range 0–1023.
    """
    clamped = int(max(0, min(1023, raw)))

    if DEBUG_CLAMP and joint_name == "MR_hip":
        print(f"[DEBUG CLAMP] {joint_name}: raw_in={raw}, raw_out={clamped}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Robust automated hip probe with detailed feedback.")

    parser.add_argument("--port", default=DEFAULT_PORT, help="COM port. Default COM6.")

    parser.add_argument("--step", type=float, default=DEFAULT_STEP_DEG, help="Step degrees. Default 8.")
    parser.add_argument("--max-angle", type=float, default=DEFAULT_MAX_ANGLE, help="Max probe angle each side. Default 95.")
    parser.add_argument("--speed", type=int, default=DEFAULT_SPEED, help="Moving speed. Default 65.")
    parser.add_argument("--settle", type=float, default=DEFAULT_SETTLE_SECONDS, help="Seconds after each step. Default 0.35.")
    ...
    parser.add_argument(
        "--debug-clamp",
        action="store_true",
        help="Print clamp input/output for MR_hip to confirm old raw limit is not being used.",
    )
...
def main():
    ...
    print("\n===================================================")
    print(" HEXAPOD ROBUST AUTOMATED HIP PROBE")
    print("===================================================")
    ...
    print(f"Debug clamp : {args.debug_clamp}")
    print("===================================================")
    print("This script ignores old joint-specific min_raw/max_raw.")
    print("It only clamps raw to absolute Dynamixel range 0–1023.")
    print("If MR_hip is tested with --debug-clamp, raw_out should NOT be stuck at 598.")
    print("===================================================")
    ...
    if not args.yes:
        confirm = input("Type y to start hip automatic probe: ").strip().lower()
